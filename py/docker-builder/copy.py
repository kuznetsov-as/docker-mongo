...
def restart():
    with open('restart.sh', 'r') as file:
        tmp = file.read()
        tmp = tmp.replace('{container_name}', container_name)
        tmp = tmp.replace('{dockerhub_login}', dockerhub_login)
        run_script(tmp)
    print_header('RESTART OK')
...
def copy_folder():
    ...
    print('*** COPY TMP OK ***\n')
    print_header(f'COPY TO {src_folder} TMP OK')
